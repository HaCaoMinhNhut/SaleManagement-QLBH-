    ...
    public class BUS_Mail
    {
        ...
        public string SendMail(string recipientEmail, string recipientPassword, bool isUpdate = false)
        {
            try
            {
                ...
                if (isUpdate)
                {
                    mailMsg.Body = "Chào bạn, mật khẩu mới truy cập vào phần mềm quản lý bán hàng của bạn là: " + recipientPassword;
                    mailMsg.Subject = "Bạn đã yêu cầu cấp lại mật khẩu!";
                }
                else
                {
                    mailMsg.Body = string.Format("Chào mừng bạn đã được thêm vào danh sách nhân viên của phần mềm quản lý bán hàng với " +
